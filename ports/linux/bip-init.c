...
uint16_t bip_receive(
    BACNET_ADDRESS *src, uint8_t *npdu, uint16_t max_npdu, unsigned timeout, dl_rc *rc)
{
    ...
    if (s > 0) {
        ...
        if (rc) *rc = (s == 0 ? DL_SEL_TIMEOUT : DL_SEL_ERROR);
        return 0;
    }
    /* See if there is a problem */
    if (received_bytes < 0) {
        if (rc) *rc = DL_RCV_ERROR;
        return 0;
    }
